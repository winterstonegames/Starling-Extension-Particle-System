...
package starling.extensions
{
	...
	public class PDParticleSystem extends ParticleSystem
	{
		...
		private var mStart3DYRotation:Number;                  // rotationStart
		private var mStart3DYRotationVariance:Number;          // rotationStartVariance
		private var mEnd3DYRotation:Number;                    // rotationEnd
		private var mEnd3DYRotationVariance:Number;            // rotationEndVariance
		...
		public function PDParticleSystem(config:XML, texture:Texture, regions:Vector.<Rectangle>)
		{
			parseConfig(config);

			var emissionRate:Number = mMaxNumParticles / mLifespan;
			super(texture, regions, emissionRate, mMaxNumParticles, mMaxNumParticles,
		...
		protected override function initParticle(aParticle:Particle):void
		{
			...
			var start3DYRotation:Number = mStart3DYRotation + mStart3DYRotationVariance * (Math.random() * 2.0 - 1.0); 
			var end3DYRotation:Number   = mEnd3DYRotation   + mEnd3DYRotationVariance   * (Math.random() * 2.0 - 1.0);

			particle.rotation3DY = start3DYRotation;
			particle.rotation3DYDelta = (end3DYRotation - start3DYRotation) / lifespan;
		}
		...
		protected override function advanceParticle(aParticle:Particle, passedTime:Number):void
		{
			...
			particle.rotation3DY += particle.rotation3DYDelta * passedTime;
		...
		private function parseConfig(config:XML):void
		{
			...
			mStart3DYRotation = deg2rad(getFloatValue(config.rotation3DYStart));
			mStart3DYRotationVariance = deg2rad(getFloatValue(config.rotation3DYStartVariance));
			mEnd3DYRotation = deg2rad(getFloatValue(config.rotation3DYEnd));
			mEnd3DYRotationVariance = deg2rad(getFloatValue(config.rotation3DYEndVariance));
