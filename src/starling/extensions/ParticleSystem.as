...
package starling.extensions
{
	...
	public class ParticleSystem extends DisplayObject implements IAnimatable
	{
		...
		private var regions:Vector.<Rectangle>;

		public function ParticleSystem(texture:Texture, regions:Vector.<Rectangle>, emissionRate:Number, 
			initialCapacity:int=128, maxCapacity:int=16383,
			blendFactorSource:String=null, blendFactorDest:String=null)
		{
			if (texture == null) throw new ArgumentError("texture must not be null or empty");
		...
		public function advanceTime(passedTime:Number):void
		{
			...
			for (var i:int=0; i<mNumParticles; ++i)
			{
				...
				var rotMaxOneRot:Number = particle.rotation3DY % 2*Math.PI;
				var regionIndex:int = rotMaxOneRot/(2*Math.PI/regions.length);
				if(regionIndex == regions.length) {
					regionIndex = 0;
				}
				var region:Rectangle = regions[regionIndex];
				var textureWidth:Number = region.width * mTexture.width;
				var textureHeight:Number = region.height * mTexture.height;				
				...
				mVertexData.setTexCoords(vertexID, region.x, region.y);
				mVertexData.setTexCoords(vertexID+1, region.x+region.width, region.y);
				mVertexData.setTexCoords(vertexID+2, region.x, region.y+region.height);
				mVertexData.setTexCoords(vertexID+3, region.x+region.width, region.y+region.height);
			}
		}
